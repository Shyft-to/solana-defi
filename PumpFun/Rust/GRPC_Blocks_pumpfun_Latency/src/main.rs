        ...
        match message {
            Ok(msg) => {
                match msg.update_oneof {
                    ...
                    let slot_received = slot.clone() as i64;
                    let slot_onchain = latest_block::get_latest_slot().unwrap() as i64;
                ...
                > = msg.transactions;
                    for txn in transactions {
                      ...
                      let latency_test = latency_test::run_latency(slot_onchain,slot_received); 
                      let formatted_time = latency_test::time_difference(block_time,block_time_received);
                       info!("
                        Time Each Block: {:?} ms
                        Block Processing: {:?}
                        Block Onchain: {:?}
                        Difference: {:?}
                       ",formatted_time,slot_received,slot_onchain,latency_test);
