...
use crate::processor::parser::PumpAmmSwapOutput;
...
impl TransactionProcessor {
    pub fn new() -> anyhow::Result<Self> {
        ...
        let pumpfun_idl = fs::read_to_string("idls/pump_amm_0.1.0.json")
        ...
        Ok(Self {
            ...
            pumpfun_program_id: Pubkey::from_str(PUMPFUN_AMM_PROGRAM_ID)?,
    ...
    ) -> anyhow::Result<Option<PumpAmmSwapOutput>> {
      ...
      let txn_info: Option<yellowstone_grpc_proto::prelude::SubscribeUpdateTransactionInfo> = update.transaction;
        ...
        if let Some(txn_info) = txn_info {
            ...
            let parsed_txn = Self::build_parsed_transaction(
                slot,
                &confirmed_txn,
                decoded_compiled.clone(),
                decoded_inner.clone(),
                block_time,
            )?;
            let parsed_accounts = self.parse_swap_transaction_output(parsed_txn,decoded_inner.clone(),decoded_compiled.clone());
            Ok(parsed_accounts)
        } else {
            Ok(None)
     }
