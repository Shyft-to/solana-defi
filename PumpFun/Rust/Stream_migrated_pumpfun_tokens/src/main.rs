...
use {
    ...
    pump_migration_interface::instructions::PumpProgramIx ,
...
const PUMPFUN_MIGRATION_PROGRAM_ID: &str = "39azUYFWPz3VHgKCf3VChUwbpURdCHRxjWVowf5jUJjg";
    ...
    pub fn get_txn_updates(&self) -> anyhow::Result<SubscribeRequest> {
        ...
        transactions.insert(
            ...
            SubscribeRequestFilterTransactions {
                ...
                account_exclude: vec![],
                account_required: vec![PUMPFUN_MIGRATION_PROGRAM_ID.to_string()],
                signature: None,
            },
        );
            ...
            Ok(msg) => match msg.update_oneof {
                Some(UpdateOneof::Transaction(update)) => {
                    ...
                    if let Some(txn) = update {
                        ...
                        let pumpfun_idl = fs::read_to_string("idls/pump_fun_0.1.0.json")
                    ...
                    let parsed_confirmed_txn_with_meta = ParsedConfirmedTransactionWithStatusMeta {
                     ...
                     let pump_amm_formatter_txn = pump_amm_formatter(parsed_confirmed_txn_with_meta);
                     if let Some(pump_amm_formatter_txn) = pump_amm_formatter_txn {
                         info!("Pumpfun AMM Formatter:\n{:#?}", pump_amm_formatter_txn);
                       }
                     }
                }
...
pub fn pump_amm_formatter(
    ...
    let binding = vec![]; 
    let log_messages = meta.log_messages.as_ref().unwrap_or(&binding);

    if log_messages.iter().any(|log| log.contains("InitializeMint2")) {
        return Some(ParsedConfirmedTransactionWithStatusMeta {
        slot: original.slot,
