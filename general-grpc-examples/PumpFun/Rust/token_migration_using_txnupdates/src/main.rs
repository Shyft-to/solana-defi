...
use {
    backoff::{future::retry, ExponentialBackoff}, clap::Parser as ClapParser, futures::{
        ...
        sink::SinkExt,
        stream::StreamExt,
    }, instruction_account_mapper::{AccountMetadata, Idl, InstructionAccountMapper}, log::{error, info}, pump_interface::instructions::PumpProgramIx, serde::{de, Serialize}, serialization::{serialize_option_pubkey, serialize_pubkey}, solana_account_decoder_client_types::token::UiTokenAmount, solana_sdk::{
...
const MIGRATION_ID: &str = "39azUYFWPz3VHgKCf3VChUwbpURdCHRxjWVowf5jUJjg";
    ...
    pub fn get_txn_updates(&self) -> anyhow::Result<SubscribeRequest> {
        ...
        transactions.insert(
            ...
            SubscribeRequestFilterTransactions {
                vote: Some(false),
                failed: Some(false),
                account_include: vec![MIGRATION_ID.to_string()],
                account_exclude: vec![],
            ...
            Ok(msg) => match msg.update_oneof {
                Some(UpdateOneof::Transaction(update)) => {
                    ...
                    if let Some(txn) = update {
                        ...
                        info!("signature Received: {}", bs58::encode(&raw_signature).into_string());
                        ...
                        if !matches!(
                            ...
                            TransactionWithStatusMeta::Complete(tx_meta) if search_for_initialize2(&tx_meta.meta, &signature).is_some()
                        ) {
                            continue;
                        }
                        println!("New signature with migration: {}", bs58::encode(&signature).into_string());
                        ...
                        decoded_txn.iter().for_each(|instruction| {
                                ...
                                .expect("Failed to parse public key")
                        {
                            match PumpProgramIx::deserialize(&instruction.instruction.data) {
                                        ...
                                        .map_accounts(&instruction.instruction.accounts, &decoded_ix.name())
                                    {
                                        Ok(mapped_accounts) => {
                                            ...
                                            match serde_json::to_string_pretty(&decoded_instruction) {
                                                Ok(json_string) => decoded_actions.push(decoded_instruction),
                                ...
                                match TokenInstruction::unpack(&instruction.instruction.data) {
                                    Ok(decoded_ix) => {
                                        ...
                                        {
                                            Ok(mapped_accounts) => {
                                                // let mint_pubkey = mapped_accounts
                                                //     .iter()
                                                ...
                                                //     if mint != sol_mint {
                                                //         println!("Token being migrated here: {}", mint);
                                                //     }
                                                // }
                                                ...
                                                match serde_json::to_string_pretty(&decoded_instruction) {
                                                    Ok(json_string) => {
                                                        ...
                                                        if decoded_instruction.name == "initializeAccount" {
                                                            if let Some(mint_account) = decoded_instruction.accounts.iter().find(|a| a.name == "mint") {
                                                                if mint_account.pubkey != sol_mint {
                                                                    println!("Token Currently in migration to Raydium: {}", mint_account.pubkey);
                                                                }
                                                            }
                                                        }
                            ...
                            // else {
                            //     // process more instrcutions when it's NOT a PumpProgram or Token instruction
                            //     match serde_json::to_string_pretty(&instruction) {
                            //         Ok(json_string) => info!("Non-PumpProgram Instruction:\n"),
                            //         Err(e) => error!("Failed to serialize non-PumpProgram instruction: {:?}", e),
                            //     }                                
                            // }
                        }); //end of foreach
...
fn search_for_initialize2(transaction_meta: &TransactionStatusMeta, signature: &Signature) -> Option<Signature> {

    let binding = vec![]; 
    // Extract log messages from transaction meta
    let log_messages = transaction_meta.log_messages.as_ref().unwrap_or(&binding);

    // Check if any log message contains 'initialize2'
    if log_messages.iter().any(|log| log.contains("initialize2")) {
