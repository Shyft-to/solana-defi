...
impl TransactionProcessor{
    ...
    pub fn decode_single_instruction(
        ...
        if instruction.instruction.program_id == self.pumpamm_program_id {
            self.decode_pumpamm_instruction(instruction, decoded_event)
    ...
    pub fn decode_pumpamm_instruction(
        &self,
        instruction: &TransactionInstructionWithParent,
        decoded_event: &Option<DecodedEvent>,
    ) -> anyhow::Result<Option<DecodedInstruction>> {
        match PumpfunAmmProgramIx::deserialize(&instruction.instruction.data) {
            Ok(decoded_ix) => {
                let mapped_accounts = self.pumpamm_idl.map_accounts(
