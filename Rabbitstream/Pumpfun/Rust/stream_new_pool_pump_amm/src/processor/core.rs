...
pub struct TransactionProcessor {
    pub pumpfun_idl: Idl,
    pub token_idl: Idl,
    pub PUMPFUN_AMM_PROGRAM_ID: Pubkey,
...
impl TransactionProcessor {
    pub fn new() -> anyhow::Result<Self> {
        ...
        let pumpfun_idl = fs::read_to_string("idls/pump_amm.json")
            .context("Unable to read Pump IDL JSON file")?;

        Ok(Self {
            pumpfun_idl: serde_json::from_str(&pumpfun_idl)?,
            token_idl: serde_json::from_str(&token_idl_json)?,
            PUMPFUN_AMM_PROGRAM_ID: Pubkey::from_str(PUMPFUN_AMM_PROGRAM_ID)?,
    ...
    update: SubscribeUpdateTransaction,
     ...
     if let Some(txn_info) = txn_info {
        ...
        let parsed_txn = Self::build_parsed_transaction(
            ...
            Ok(self.parsed_pump_amm_txn(parsed_txn))
        } else {
            Ok(None)
     }
}
