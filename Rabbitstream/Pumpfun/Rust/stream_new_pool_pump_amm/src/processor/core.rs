...
use crate::PUMPFUN_AMM_PROGRAM_ID;
...
impl TransactionProcessor {
    pub fn new() -> anyhow::Result<Self> {
        ...
        let pumpamm_idl = fs::read_to_string("idls/pump_amm_0.1.0.json")
            .context("Unable to read Pump IDL JSON file")?;

        Ok(Self {
            pumpamm_idl: serde_json::from_str(&pumpamm_idl)?,
            token_idl: serde_json::from_str(&token_idl_json)?,
            pumpamm_program_id: Pubkey::from_str(PUMPFUN_AMM_PROGRAM_ID)?,
  ...
  pub fn process_transaction_update(
    ...
    update: SubscribeUpdateTransaction,
    ) -> anyhow::Result<(Option<Signature>,Option<PumpAmmCreatePoolEvent>)> {
     let slot = update.slot;
     let block_time = SystemTime::now()
        .duration_since(UNIX_EPOCH)?
        .as_secs() as i64;

     let txn_info = update.transaction;
     ...
     if let Some(txn_info) = txn_info {
        let signature = Self::parse_signature(&txn_info.signature)
         ...
         let meta = if let Some(meta) = txn_info.meta {
            meta
        } else {
        ...
        let pump_amm = self.parsed_pump_amm_txn(parsed_txn);

    Ok((Some(signature), pump_amm))
    } else {
        Ok((None,None))
    }
}
    ...
    fn compiled_instruction_to_instruction(
        ...
        return Instruction { program_id: Pubkey::default(), accounts: vec![], data: vec![] };
        };

        let accounts: Vec<AccountMeta> = ci.accounts.iter().filter_map(|&index| {
