...
A Rust service that streams real-time Solana transactions via Rabbitstream and detects new pool creations on the PumpFun AMM by filtering txn using its instruction name `CreatePool`.

## Key Features
- Real-time monitoring of Solana blockchain via Rabbitstream WebSocket
