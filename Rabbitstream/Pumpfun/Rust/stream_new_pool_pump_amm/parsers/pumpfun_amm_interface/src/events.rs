...
#[derive(Clone, Debug, PartialEq)]
pub struct PumpAmmCreatePoolEventEvent(pub PumpAmmCreatePoolEvent);

impl BorshSerialize for PumpAmmCreatePoolEventEvent {
    fn serialize<W: io::Write>(&self, writer: &mut W) -> io::Result<()> {
...
impl PumpAmmCreatePoolEventEvent {
    pub fn deserialize(buf: &mut &[u8]) -> io::Result<Self> {
        let maybe_discm = <[u8; 8]>::deserialize(buf)?;
        ...
        Ok(Self(PumpAmmCreatePoolEvent::deserialize(buf)?))
    }
}
