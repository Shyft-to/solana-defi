...
export class PumpAmmDecoder {
  ...
  try {
    ...
    if (!parsedInnerIxs.length) return null;
    ...
    const { pumpfun_inner_ixs } = this.decodeAndUpdatePumpInstructions(pump_amm_inner_ixs);
    ...
    const ensurePublicKey = (arr?: any[]): PublicKey[] => {
     ...
     return arr.map((item: any) => {
       try {
         ...
         if (Buffer.isBuffer(item) || item instanceof Uint8Array) return new PublicKey(item);
