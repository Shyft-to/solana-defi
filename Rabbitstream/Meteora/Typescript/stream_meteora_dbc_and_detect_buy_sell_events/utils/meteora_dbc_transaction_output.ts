export function meteoradbcTransactionOutput(parsedInstruction) {
  const ixns = parsedInstruction?.instructions;
  ...
  const params = swapTxn?.args?.params ?? {};

  const getAccount = (name) =>
    swapTxn.accounts.find((a) => a.name === name)?.pubkey;

  const baseMint = getAccount("base_mint");
  const quoteMint = getAccount("quote_mint");
