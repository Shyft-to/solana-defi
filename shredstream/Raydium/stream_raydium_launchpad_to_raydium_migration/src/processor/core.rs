                ...
                if !matching_entries.is_empty() {
                    ...
                    matching_entries.iter().for_each(|entry| {
                        entry.transactions.iter().for_each(|tx| {
                           ...
                           if let Some(migrated_transaction) = rl_formatter(parsed_tx){
                              println!("{:#?}", migrated_transaction);
                           }
                        });
                    });
                }
...
fn rl_formatter(parsed_transaction: ParsedTransaction) ->  Option<ParsedTransaction> {
    let migration_instruction = parsed_transaction.message.instructions.iter().find(|instr|{
           if let Some(DecodedInstruction { name, .. })  = &instr.data {
              name == "migrate_to_amm" || name == "migrate_to_cpswap"
           } else {
             false
           }                                         
       });
       
     if let Some(_) = migration_instruction {
