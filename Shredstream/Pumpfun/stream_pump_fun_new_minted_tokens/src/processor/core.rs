...
pub async fn connect_and_stream(
    ...
    println!("Following on Minted Tokens on Pumpfun");
        ...
        match result {
            Ok(slot_entry) => {
                ...
                if !matching_entries.is_empty() {
                    ...
                    let pump_idl = fs::read_to_string("idls/pump_0.1.0.json")
                        .expect("Unable to read IDL JSON file");

                    let idl: Idl = serde_json::from_str(&pump_idl)
                        ...
                        entry.transactions.iter().for_each(|tx| {
                            ...
                            let parsed_instructions: Vec<CompiledInstruction> = tx
                                ...
                                .map(|ci| {
                                    ...
                                    let decoded = match PumpProgramIx::deserialize(
                                        &instruction.data,
                                    ) {
                                        Ok(decoded_ix) => {
                            ...
                            if let Some(pump_fun_formatter) = pump_amm_formatter(parsed_tx){
                             println!("{:#?}", pump_fun_formatter);
                            }
                        });
                    });
                }
...
fn pump_amm_formatter(parsed_txn: ParsedTransaction) -> Option<PumpfunEventType> {
    ...
    if let Some(instruction) = create_instruction {
        if let Some(DecodedInstruction { data: serde_json::Value::Object(data), accounts, .. }) = &instruction.data {
            if let Some(serde_json::Value::Object(create_data)) = data.get("Create") {
                let token_name = create_data.get("name")
                    .and_then(|v| v.as_str())
                    .unwrap_or_default()
                    .to_string();

                let token_symbol = create_data.get("symbol")
                    .and_then(|v| v.as_str())
                    .unwrap_or_default()
                    .to_string();

                let token_uri = create_data.get("uri")
                    .and_then(|v| v.as_str())
                    .unwrap_or_default()
                    .to_string();

                // Creator is stored as a byte array
                let creator_bytes = create_data.get("creator")
