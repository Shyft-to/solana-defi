...
export class WideBits<P extends string = ""> extends Layout<Record<string, boolean>, P> {
  ...
  constructor(property?: P) {
    //@ts-expect-error type wrong for super()'s type different from extends 
    super(8, property);
...
export function tagged<T, P extends string = "">(tag: BN, layout: Layout<T>, property?: P): Layout<T, P> {
  const wrappedLayout: Layout<{ tag: BN; data: T }> = struct([u64("tag"), layout.replicate("data")]) as any; 
...
export function vecU8<P extends string = "">(property?: P): Layout<Buffer, P> {
  const length = _u32("length");
  const layout: Layout<{ data: Buffer }> = struct([length, blob(_offset(length, -length.span), "data")]) as any; 
  return new WrappedLayout(
    layout,
    ({ data }) => data,
    (data) => ({ data }),
...
export function array<T, P extends string = "">(
  ...
  const layout = struct([seq(elementLayout, length, "values")]) as any as Layout<{ values: T[] }>; 
  return new WrappedLayout(
    layout,
    ({ values }) => values,
    (values) => ({ values }),
...
export function struct<T, P extends string = "">(
  ...
  >
  : any {
  //@ts-expect-error this type is not quite satisfied the define 
...
export function seq<T, P extends string = "", AnotherP extends string = "">(
  ...
  const superCount =
    typeof count === "number"
      ...
      : isBN(count)
        ? count.toNumber()
        : new Proxy(count as unknown as Layout<number> , {
