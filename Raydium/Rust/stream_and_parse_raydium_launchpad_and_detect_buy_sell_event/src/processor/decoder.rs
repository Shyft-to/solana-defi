...
use raydium_launchpad_interface::instructions::RaydiumLaunchpadProgramIx;
...
impl TransactionProcessor{
    ...
    pub fn decode_single_instruction(
        ...
        if instruction.instruction.program_id == self.raydium_launchpad_program_id {
            self.decode_raydium_launchpad_instruction(instruction, decoded_event)
    ...
    pub fn decode_raydium_launchpad_instruction(
        &self,
        instruction: &TransactionInstructionWithParent,
        decoded_event: &Option<DecodedEvent>,
    ) -> anyhow::Result<Option<DecodedInstruction>> {
        match RaydiumLaunchpadProgramIx::deserialize(&instruction.instruction.data) {
            Ok(decoded_ix) => {
                let mapped_accounts = self.raydium_launchpad_idl.map_accounts(
            ...
            Err(e) => {
                log::error!("Failed to decode Raydium instruction: {:?}", e);
                Ok(None)
            }
        }
    }
