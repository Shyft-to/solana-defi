...
impl TransactionProcessor {
    ...
    pub fn parsed_raydium_launchpad_txn(
        &self,
     ...
     .find(|instr| instr.name == "BuyExactIn" 
                || instr.name == "BuyExactOut"
                || instr.name == "SellExactIn"
                || instr.name == "SellExactOut" )?;
     
     
     let mint_pubkey = launchpad_instruction
                    .accounts
                    .iter()
                    .find(|acc| acc.name == "base_token_mint")
     ...
     let (amount_in,amount_out, quote_amount_before,base_amount_before, quote_amount_after,base_amount_after, trade_direction) = match traded_event {
            DecodedEvent::TradeEvent(event) => (
             event.amount_in,
             event.amount_out,
    ...
    let event = TransactionEvent {
        event_type: trade_direction.to_string(),
        user: signer_pubkey,
        mint: mint_pubkey,
        amount_in: Some(amount_in),
        amount_out: Some(amount_out),
        quote_amount_before: Some(quote_amount_before),
        base_amount_before: Some(base_amount_before),
        quote_amount_after: Some(quote_amount_after),
        base_amount_after: Some(base_amount_after),
    };
