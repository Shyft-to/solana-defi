...
use {
    ...
    instruction_account_mapper::{AccountMetadata, Idl, InstructionAccountMapper},
     ...
     raydium_clmm_swap_interface::instructions::RaydiumClmmSwapProgramIx ,
            ...
            Ok(msg) => match msg.update_oneof {
                Some(UpdateOneof::Transaction(update)) => {
                    ...
                    if let Some(txn) = update {
                        ...
                        let clmm_idl_raw = fs::read_to_string("idls/raydium_clmm.json")
                        .expect("Unable to read IDL JSON file");
                        let clmm_idl = clmm_idl_raw.trim_start_matches('\u{feff}');
                        let clmm_idl = clmm_idl.to_string();
                        ...
                        compiled_instructions.iter().for_each(|instruction| {
                            if instruction.instruction.program_id
                                == Pubkey::from_str(RAYDIUM_CLMM_PROGRAM_ID)
                                    .expect("Failed to parse public key")
                            {
                                match RaydiumClmmSwapProgramIx::deserialize(&instruction.instruction.data) {
                                    Ok(decoded_ix) => {
                                        let idl: Idl =
                                            serde_json::from_str(&clmm_idl).expect("Failed to deserialize IDL"); 
                                        match idl
