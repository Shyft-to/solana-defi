...
use meteora_dbc_interface::instructions::MeteoraDBCProgramIx;
...
impl TransactionProcessor{
    pub fn decode_instructions(
        ...
        for instruction in compiled_instructions {
            if let Ok(Some(decoded)) = self.decode_single_instruction(instruction, decoded_event) {
    ...
    pub fn decode_single_instruction(
        ...
        if instruction.instruction.program_id == self.meteora_dbc_program_id {
            self.decode_meteora_dbc_instruction(instruction, decoded_event)
    ...
    pub fn decode_meteora_dbc_instruction(
        &self,
        instruction: &TransactionInstructionWithParent,
        decoded_event: &Option<DecodedEvent>,
    ) -> anyhow::Result<Option<DecodedInstruction>> {
        match MeteoraDBCProgramIx::deserialize(&instruction.instruction.data) {
            Ok(decoded_ix) => {
                let mapped_accounts = self.meteora_dbc_idl.map_accounts(
