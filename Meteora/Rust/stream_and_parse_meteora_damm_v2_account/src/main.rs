...
pub fn decode_account_data(buf: &[u8]) -> Result<DecodedAccount, AccountDecodeError> {
    ...
    match discriminator {
        CLAIM_FEE_OPERATOR_ACCOUNT_DISCM => {
            let data = ClaimFeeOperatorAccount::deserialize(buf)
        ...
        POSITION_ACCOUNT_DISCM => {
         match PositionAccount::deserialize(buf) {
            Ok(data) => Ok(DecodedAccount::Position(data.0)),
              Err(_) => {
         ...
         VESTING_ACCOUNT_DISCM => {
            let data = VestingAccount::deserialize(buf)
                .map_err(|e| AccountDecodeError {
