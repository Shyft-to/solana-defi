...
use {
    ...
    instruction_account_mapper::{AccountMetadata, Idl, InstructionAccountMapper},
     ...
     meteora_dlmm_interface::instructions::MeteoraDlmmProgramIx,
            ...
            Ok(msg) => match msg.update_oneof {
                Some(UpdateOneof::Transaction(update)) => {
                    ...
                    if let Some(txn) = update {
                        ...
                        let meteora_idl_json = fs::read_to_string("idls/meteora_dlmm.json").expect("Unable to read IDL JSON file");

                        decoded_txn.iter().for_each(|instruction| {

                            if instruction.instruction.program_id
                            == Pubkey::from_str(METEORA_DLMM_PROGRAM_ID)
                                .expect("Failed to parse public key")
                        {
                            match MeteoraDlmmProgramIx::deserialize(&instruction.instruction.data) {
                                Ok(decoded_ix) => {
                                        ...
                                        .map_accounts(&instruction.instruction.accounts, &decoded_ix.name())
                                    {
                                        Ok(mapped_accounts) => {
                                            ...
                                            match serde_json::to_string_pretty(&decoded_instruction) {
                                                Ok(json_string) => {
                                                    info!("Decoded Meteora Dlmm Instruction:\n{}", json_string);
