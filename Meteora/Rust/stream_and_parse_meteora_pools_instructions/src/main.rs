...
use {
    ...
    instruction_account_mapper::{AccountMetadata, Idl, InstructionAccountMapper},
     ...
     meteora_pools_interface::instructions::MeteoraPoolsProgramIx,
...
const METEORA_POOLS_PROGRAM_ID: &str = "Eo7WjKq67rjJQSZxS6z3YkapzY3eMj6Xy8X5EQVn5UaB";
    ...
    pub fn get_txn_updates(&self) -> anyhow::Result<SubscribeRequest> {
        ...
        transactions.insert(
            ...
            SubscribeRequestFilterTransactions {
                vote: Some(false),
                failed: Some(false),
                account_include: vec![METEORA_POOLS_PROGRAM_ID.to_string()],
                account_exclude: vec![],
            ...
            Ok(msg) => match msg.update_oneof {
                Some(UpdateOneof::Transaction(update)) => {
                    ...
                    if let Some(txn) = update {
                        ...
                        let meteora_idl_json = fs::read_to_string("idls/meteora_pools.json").expect("Unable to read IDL JSON file");

                        decoded_txn.iter().for_each(|instruction| {

                            if instruction.instruction.program_id
                            == Pubkey::from_str(METEORA_POOLS_PROGRAM_ID)
                                .expect("Failed to parse public key")
                        {
                            match MeteoraPoolsProgramIx::deserialize(&instruction.instruction.data) {
                                Ok(decoded_ix) => {
                                    //println!("Decoded Raydium Instruction: {:?}", decoded_ix);

                                    let idl: Idl =
                                        serde_json::from_str(&meteora_idl_json).expect("Failed to deserialize IDL");
                                        ...
                                        .map_accounts(&instruction.instruction.accounts, &decoded_ix.name())
                                    {
                                        Ok(mapped_accounts) => {
                                            ...
                                            match serde_json::to_string_pretty(&decoded_instruction) {
                                                Ok(json_string) => {
                                                    info!("Decoded Meteora Pool Instruction:\n{}", json_string);
